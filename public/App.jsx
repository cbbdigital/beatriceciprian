...
function DashboardPage({ onLogout }) {
  ...
  const updatePersons = async (id, adults, children) => {
    ...
    if (existing && existing.length > 0) {
      await fetch(`${SUPABASE_URL}/rest/v1/rsvp_responses?guest_id=eq.${id}`, {
        method: 'PATCH',
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ adults: parseInt(adults)||0, children: parseInt(children)||0 })
      });
    }
    setGuests(prev => prev.map(g => g.id === id ? { ...g, adults: parseInt(adults)||0, children: parseInt(children)||0 } : g));
  };
                      ...
                      <td style={{ padding:'0.5rem 0.9rem' }}>
                        {g.attending === true ? (
                          <div style={{ display:'flex', alignItems:'center', gap:'0.3rem' }}>
                            <input type="number" min="0" max="20" defaultValue={g.adults||1}
                              onBlur={e => updatePersons(g.id, e.target.value, g.children||0)}
                              style={{ width:32, padding:'0.2rem', border:`1px solid ${S.border}`, fontFamily:'inherit', fontSize:'0.78rem', textAlign:'center', outline:'none', color:S.text }}
                            />
                            <span style={{ fontSize:'0.6rem', color:S.textLight }}>a</span>
                            <input type="number" min="0" max="20" defaultValue={g.children||0}
                              onBlur={e => updatePersons(g.id, g.adults||1, e.target.value)}
                              style={{ width:32, padding:'0.2rem', border:`1px solid ${S.border}`, fontFamily:'inherit', fontSize:'0.78rem', textAlign:'center', outline:'none', color:S.text }}
                            />
                            <span style={{ fontSize:'0.6rem', color:S.textLight }}>c</span>
                          </div>
